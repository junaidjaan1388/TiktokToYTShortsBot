...
export async function GetReelDetailsV2(ReelUrl:string)
{
  ...
  if (match && match[1]) {
      ...
      try
      {
          const response = await axios<igTypes>({
              url : API_URL,
              method : 'POST',
              headers,
              data:encodedData
          ...
          const retunedData = {
            reelUrl : ReelUrl,
            title : response.data.data.xdt_shortcode_media.edge_media_to_caption.edges[0].node.text,
            description : get_hashtags(response.data.data.xdt_shortcode_media.edge_media_to_caption.edges[0].node.text).join(' ') ,
            dynamic_cover  : response.data.data.xdt_shortcode_media.display_url,
            duration : response.data.data.xdt_shortcode_media.video_duration.toFixed(2),
            video : response.data.data.xdt_shortcode_media.video_url
          }
...
//GetReelDetailsV2("https://www.instagram.com/reels/Cz4NR-KIu_H/",false)
